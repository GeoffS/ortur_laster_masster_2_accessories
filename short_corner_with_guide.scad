...
module basePlate()
{
	difference()
    {
        union()
        {
            ...
            hull()
            {
                translate([pbx, pby, 0]) simpleChamferedCylinder(d=baseCornerDia, h=baseZ, cz=baseCZ);
                translate([pbx,  15, 0]) simpleChamferedCylinder(d=baseCornerDia, h=baseZ, cz=baseCZ);
            }
            // Guide:
            hull()
            {
        ...
        // Corner relief hole:
        cornerReliefDia = 4;
        cornerReliefX = pbx-baseCornerDia/2; //+cornerReliefDia/2;
        translate([cornerReliefX, 0, -1]) hull()
        {
            h = baseZ + 1 + nothing;
            cylinder(d=cornerReliefDia, h=h);
            tcy([-10, 10, 0], d=cornerReliefDia, h=h);
        }
